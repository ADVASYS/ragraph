...
2. **Relevance gate.** A small sub-LLM call (`temperature: 0`, hidden chat model) receives the current goal (last user message or an explicit `subGoal`) plus every candidate and decides what the main model gets to see. The gate tries three strategies, each with its own `DEFAULT_TIMEOUT_MS = 6s` budget, in order:
   1. **Structured (`generateObject` + Zod).** Best on capable providers — returns `{ kept: [{ sourceId, relevance, why }], droppedIds: [] }` directly.
   2. **Line-list prose.** Ultra-simple `#<n> high|medium|low|drop: <why>` one-line-per-candidate format, parsed with a lenient regex. This is the format that small / local / non-JSON-mode providers can actually produce reliably.
   3. **Loose JSON prose.** Last-ditch JSON extractor for the rare provider that refuses the line-list format but does emit a usable JSON blob.

   If all three strategies fail the gate falls back to keeping every candidate at `medium` with a `"relevance gate unavailable"` reason, so the agent loop never stalls on a flaky provider. A module-scoped **circuit breaker** (`STRUCTURED_FAILURE_COOLDOWN = 2`) disables the `generateObject` attempt for `STRUCTURED_DISABLE_MS = 30m` after two consecutive failures, avoiding a ~6 s wasted attempt on every heavy tool call when the provider demonstrably cannot produce structured output. A single success re-enables it.
...
Every tool is wrapped in `Agent.wrapTools`, which stacks four independent guards on top of every call:

1. **Per-tool call-count budget** (coarse, args-independent). Each tool has a hard cap per turn — roughly `vectorSearch ≤ 6`, `entitySearch` / `findEntityMentions` / `findRelatedDocs` / `graphNavigate ≤ 4`, `navigate ≤ 6`, `quote` / `inspect` ≤ `10`, `summarizeSubthread ≤ 3`. Calls over the limit return `{ error: "per_tool_budget_exceeded", message }` with the concrete tool name and limit. This is the guard that catches the failure mode where the model paraphrases `subGoal` / `query` / `topK` just enough to bypass the fingerprint-based loop detector and calls the same tool 15+ times. Defaults live in `PER_TOOL_LIMITS` inside `Agent.ts`.
2. **Soft total-turn budget** (`SOFT_TOTAL_BUDGET = 18`). Once the total number of tool calls in this turn crosses the soft limit, further HEAVY exploration tools (`vectorSearch`, `entitySearch`, `findEntityMentions`, `findRelatedDocs`, `graphNavigate`, `navigate`, `sampleKnowledge`, `findPath`, `listTopics`, `listDomains`, `listEntities`, `topicHierarchy`) return `{ error: "soft_total_budget_exceeded", message }`. Drill tools (`quote`, `inspect`, `summarizeSubthread`), `getDocumentSummary` / `getChunk` and `saveAgentNote` remain usable so the model can pull in exact citations for the final answer.
3. **Fingerprint loop detection** (fine). A stable fingerprint (`tool:${stableStringify(args)}`, including `subGoal`) is counted; identical calls beyond `LOOP_LIMIT` (default `3`) return `{ error: "loop_detected", message }`. The outer stream is **not** aborted — aborting was the root cause of the agent occasionally ending a turn with an empty assistant message when the model retried a slow tool a few times in a row. Including `subGoal` in the fingerprint means a genuinely refined relevance-gate goal on an otherwise identical call does not trip the guard.
4. **Per-tool timeout.** The tool promise is raced against a `setTimeout` of `budget.toolTimeoutMs`. On timeout the tool resolves with `{ error: "tool_timeout", message }` instead of hanging the agent loop. The relevance-gate sub-LLM has its own, much smaller internal budget (`DEFAULT_TIMEOUT_MS = 6s`) so a slow gate can never consume the outer tool budget.

On top of these, an external `AbortSignal` (e.g. user clicks "stop generating") aborts the `streamText` controller; the SDK unwinds the stream cleanly and the agent still emits a final `onFinish` with the sources it has collected so far.
